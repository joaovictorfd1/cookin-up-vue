...
import CardsReceitaVue from './CardsReceita.vue';
...
export default {
  ...
  components: { CardsReceitaVue, BotaoBuscar },
  emits: ['editarLista']
}
</script>
...
<template>
  <section class="mostrar-receitas">
    ...
    <div v-if="receitas.length" class="receitas-wrapper">
      ...
      <ul class="receitas">
        <li v-for="receita of receitas" :key="receita.nome">
          <CardsReceitaVue :receitas="receita" />
        </li>
      </ul>
    </div>
    ...
    <BotaoBuscar texto="Editar lista!" @click="$emit('editarLista')" />
  </section>
</template>
